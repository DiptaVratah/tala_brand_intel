...
must_haves:
  truths:
    - "Root URL (/) shows landing page with tagline and mode selection cards"
    - "Server handles direct URL access to /branding, /author, /self-reflection without 404"
    - "Router module exists with navigate() and route() methods"
  artifacts:
...
Output: router.js module, landing page HTML/CSS structure, server wildcard route support.
</objective>
...
- `window.pulsecraftRouter` assignment

Run: `grep -E "(class PulseCraftRouter|pushState|popstate)" pulsecraft_ui/router.js`
  </verify>
  <done>router.js exists with PulseCraftRouter class exposing navigate(), route(), init() methods and popstate handling</done>
</task>
...
```
  ...
  <done>Landing page HTML structure exists with 3 mode cards, landing.css provides professional styling, app-view wrapper contains existing phases</done>
</task>
...
3. **Server route check:**
   ```bash
   grep "app.get('\*'" server.js
   ```

4. **Visual check (requires server running):**
...
<success_criteria>
- [ ] router.js exists with PulseCraftRouter class and History API integration
- [ ] landing.css exists with professional mode card styling
- [ ] index.html has landing-view section with 3 mode cards
